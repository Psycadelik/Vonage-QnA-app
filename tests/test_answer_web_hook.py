...
class TestAnswerWebHook(unittest.TestCase):
    ...
    def test_successful_answer_web_hook(self):
        ...
        response = self.app.post('/inbound/', data=json.dumps(self.answerData), content_type='application/json')
        self.assertEqual(response.status_code, 200)

    def test_failed_answer_web_hook(self):
        """ test that the answer web hook returns a failed response """
        response = self.app.post('/inbound/', data=json.dumps(self.failAnswerData), content_type="application/json")
